class PostOffice:
    ...
    def send_message(self, sender, recipient, message_body, urgent=False):
        ...
        message_details = {
            ...
            'body': message_body,
            'sender': sender,
            # Added an opened field to know if the message already seen
            'opened': False,
        }
        if urgent:
    ...
    def read_inbox(self, user_name: str, N = -1) -> list:
        ...
        for message in range(0, N):
            if not self.boxes[user_name][message]['opened']:
                n_first_messages.append(self.boxes[user_name][message])
    ...
    def search_inbox(self, user_name: str, string_to_search: str) -> list:
        ...
        """
        all_messages_contain_the_string = [self.boxes[user_name][message] for message in range(0, self.message_id)
                                           if string_to_search in self.boxes[user_name][message]['body'] or
                                           string_to_search in self.boxes[user_name][message]['sender']]
